...
internal sealed class UpdateSmtpCommandHandler(
    IApplicationDbContext applicationDbContext) : ICommandHandler<UpdateSmtpCommand>
{
    public async Task<Result> Handle(UpdateSmtpCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure($"SMTP configuration with ID {command.SmtpId} not found.");
        }
